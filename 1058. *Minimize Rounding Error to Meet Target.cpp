...
public:
    string minimizeError(vector<string>& prices, int target) {
        ...
        int round_ups = target - _min;                                                      // greedy, this is a int diff, we need to keep minmum errors.
        ...
        for (int i = 0; i < dprices.size(); ++ i) {
            if (c < round_ups && ceil (dprices[i]) - dprices[i] != 0) {
               err += (ceil (dprices[i]) - dprices[i]);                                     // greedy round up
                c++;
            }
            else if (c >= round_ups) err += (dprices[i] - floor (dprices[i]) );           // greedy round down
        }
